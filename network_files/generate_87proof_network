...
model = Sequential()
model.add(Conv2D(36, kernel_size=(3, 3), activation='relu', input_shape=x_train.shape[1:]))
...
plt.savefig('model_results/accuracy_plot87proof.png')
plt.show()
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Validation'], loc='upper left')
plt.savefig('model_results/loss_plot87proof.png')
plt.show()



# Save the model
model.save('network_files/model87proof.h5')
