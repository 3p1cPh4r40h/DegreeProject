...
# Create a second convolutional neural network model
model = Sequential()
model.add(Conv2D(12, kernel_size=(3, 3), activation='elu', input_shape=x_train.shape[1:]))
model.add(ThresholdedReLU(theta=0.3))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(Conv2D(36, kernel_size=(3, 3), activation='elu'))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(Flatten())
model.add(Dense(132, activation='elu'))
...
# Train the model
history = model.fit(x_train, y_train, epochs=100, batch_size=48, validation_data=(x_val, y_val))
...
plt.savefig('model_results/accuracy_plot90proof.png')
plt.show()
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Validation'], loc='upper left')
plt.savefig('model_results/loss_plot90proof.png')
plt.show()



# Save the model
model.save('network_files/model90proof.h5')
